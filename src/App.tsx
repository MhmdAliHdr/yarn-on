    ...
    <div className="grid grid-cols-1 gap-4 bg-[#F3F3F1] mr-auto ml-auto text-center mt-20 mb-20 shadow-lg shadow-black w-full max-w-280 mx-auto px-4 py-0">
      ...
      {currentPattern.sections.map(section =>
        ...
        // Buttons to allow adding and removing rows from a section
          <>
          <input type="button" data-html2canvas-ignore className="transition-transform duration-300 hover:scale-110 mr-5 border-2 border-black-500 rounded-full text-center mt-2 text-2xl w-13 h-13 rounded-full" value="+" onClick={addRow}></input>
          <input type="button" data-html2canvas-ignore className="transition-transform duration-300 hover:scale-110  border-2 border-black-500 rounded-full mt-2 font-bold text-2xl w-13 h-13 rounded-full" value="-" onClick={removeRow}></input>
          </> : <></>
        }
        </>
      )}
